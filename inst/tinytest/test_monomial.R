...
vanderMat <- get("vanderMat", nS, inherits = FALSE, mode = "function")
...
## Test scalar
x <- 3
expect_equal(polyCalc(x, coeffs), controlF(x), tolerance = tol)
x <- 5
expect_equal(polyCalc(x, coeffs), controlF(x), tolerance = tol)
x <- 1e-14
expect_equal(polyCalc(x, coeffs), controlF(x), tolerance = tol)
...
## Test vectorized
expect_equal(polyCalc(c(3, 5, 1e-14), coeffs),
